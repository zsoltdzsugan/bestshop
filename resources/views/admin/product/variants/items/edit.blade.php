<x-admin-layout>
    ...
    <div class="overflow-hidden w-full overflow-x-auto rounded-radius border border-outline dark:border-outline-dark p-4">
        <section>
            ...
            <form method="post" action="{{ route('admin.product.variants.items.update', ['product' => $item->variant->product->id, 'variant' => $item->variant->id, 'item' => $item->id]) }}" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('patch')
                ...
                <div class="flex items-center gap-4">
                    ...
                    <x-secondary-button :href="route('admin.product.variants.items.index', ['product' => $item->variant->product->id, 'variant' => $item->variant->id])">{{ __('Back') }}</x-secondary-button>
                </div>
            </form>
        </section>
    </div>
