<x-admin-layout>
    ...
    <div class="overflow-hidden w-full overflow-x-auto rounded-md border border-outline dark:border-outline-dark p-4">
        <section>
            ...
            <form method="post" action="{{ route('admin.sub-category.store') }}" enctype="multipart/form-data" class="space-y-6">
                @csrf

                <div>
                    <x-input-label for="Category" :value="__('Category')" />
                    <x-select-input label="Category" id="category_id" name="category_id" placeholder="Select Category">
                ...
                <div class="flex items-center gap-4">
                    ...
                    @if (session('status') === 'sub-category-created')
                        <p
                            x-data="{ show: true }"
                            x-show="show"
                            x-transition
