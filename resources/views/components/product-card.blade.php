...
<article class="group flex rounded-radius h-full flex-col overflow-hidden bg-surface text-on-surface dark:bg-surface-dark dark:text-on-surface-dark">
    ...
    <div class="relative overflow-hidden {{ $imageContainerClasses() }}">
        ...
        @endif
        @if ($isOnSale)
            <div class="font-bold group-hover:opacity-0 transition-all duration-300 ease-in-out absolute max-w-12 w-full top-2 right-2 p-1.5 bg-danger flex items-center justify-center text-center">-{{ $percent }}<span class="text-xs font-bold">%</span></div>
        @endif
    </div>
    <!-- Content -->
    <div class="flex flex-col gap-4 {{ $contentContainerClasses }}">
        ...
        <div class="flex justify-between {{ $headerClasses }}">
            ...
            <div class="flex flex-col gap-1">
                <h3 class="{{ $titleClasses() }} text-on-surface-strong dark:text-on-surface-dark-strong" aria-describedby="{{ $product->name }}">{{ $product->name }}</h3>
                <!-- Rating -->
        ...
        @if ($variant === 'default')
            ...
            <!-- Button -->
            <x-button type="button" class="flex items-center">
