<x-vendor-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl leading-tight">
            {{ __('Create Product Variant Item') }}
        </h2>
    ...
    <div class="overflow-hidden w-full overflow-x-auto rounded-radius border border-outline dark:border-outline-dark p-4">
        <section>
            ...
            <form method="post" action="{{ route('vendor.product.variants.items.store', ['product' => $product->id, 'variant' => $variant->id]) }}" enctype="multipart/form-data" class="space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <x-text-input id="variant_id" name="variant_id" type="text" hidden value="{{ $variant->id }}" />
                ...
                <div class="flex items-center gap-4">
                    ...
                    <x-secondary-button :href="route('vendor.product.variants.index', $product->id)">{{ __('Back') }}</x-secondary-button>
                </div>
            </form>
        </section>
    </div>
