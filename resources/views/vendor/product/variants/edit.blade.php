<x-vendor-layout>
    ...
    <div class="overflow-hidden w-full overflow-x-auto rounded-radius border border-outline dark:border-outline-dark p-4">
        <section>
            ...
            <form method="post" action="{{ route('vendor.product.variants.update', ['product' => $variant->product_id, 'variant' => $variant->id]) }}" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('patch')
